<template>
  <div>
    ...
    <div style="width: calc(100% - 2px);height:calc(100vh);">
      <RelationGraph ref="graphRef" :options="userGraphOptions" :on-node-expand="onNodeExpand" :on-node-collapse="onNodeCollapse" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    setGraphData(expandAll = false) {
      const _orign_data = {
        entname: '张三疯子',
        invs: [
          { id: 'inv1', text: '张三疯子', desc: '40%' },
        ...
        asInvs: [
          { id: 'asinv1', text: '路人乙', desc: '80%' },
          { id: 'asinv3', text: '路人乙', desc: '20%' }
        ],
        branchs: [
          { id: 'branch1', text: '路人乙', desc: '80%' },
          { id: 'branch5', text: '路人乙', desc: '20%' }
        ]
      }
      ...
      graphData.lines.push({ from: rootNode.id, to: invRootNode.id, color: '#C7E9FF' })
      graphData.lines.push({ from: rootNode.id, to: personRootNode.id, color: '#C7E9FF' })
      graphData.lines.push({ from: rootNode.id, to: asinvRootNode.id, color: '#C7E9FF' })
      graphData.lines.push({ from: rootNode.id, to: branchRootNode.id, color: '#C7E9FF' })
      // 将股东加入虚拟节点"股东"
      _orign_data.invs.forEach((thisNode, _index) => {
        ...
        graphData.lines.push({ from: invRootNode.id, to: thisNode.id, text: thisNode.desc, color: '#FFC5A6', arrow: 'none' })
      })
      // 将高管加入虚拟节点"高管"
      _orign_data.persons.forEach((thisNode, _index) => {
        ...
        graphData.nodes.push(thisNode)
        graphData.lines.push({ from: personRootNode.id, to: thisNode.id, text: thisNode.desc, color: '#B9FFA7', arrow: 'none' })
      })
      // 将对外投资企业加入虚拟节点"对外投资"
      _orign_data.asInvs.forEach((thisNode, _index) => {
        ...
        graphData.nodes.push(thisNode)
        graphData.lines.push({ from: asinvRootNode.id, to: thisNode.id, text: thisNode.desc, color: '#FFBEC1' })
      })
      // 将分支机构加入虚拟节点"分支机构东"
      _orign_data.branchs.forEach((thisNode, _index) => {
        ...
        thisNode.styleClass = 'my-node-class'
        graphData.nodes.push(thisNode)
        graphData.lines.push({ from: branchRootNode.id, to: thisNode.id, text: thisNode.desc, color: '#FFA1F8' })
      })
