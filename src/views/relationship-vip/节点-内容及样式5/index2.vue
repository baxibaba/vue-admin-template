  ...
  <div>
    <div style="height: calc(100vh); background-color: #222222">
      ...
      >
        ...
        <!-- node slot -->
        <template #node="{ node }">
          <SingleAIModel :node="node" :data-type-color-map="dataTypeColorMap" />
        </template>
      </RelationGraph>
    </div>
  </div>
...
export default {
  ...
  components: { SingleAIModel, SingleAIModel },
  data() {
    return {
              ...
              options: [
                {
                  name: 'model_name',
                  type: 'string',
                  value: 'diff-v1.safetensors'
                },
                {
                  name: 'beta_schedule',
                  type: 'string',
                  value: 'sqrt_linearr(AnimateDiff)'
                }
              ],
              input: [
                { name: 'model', type: 0 },
              ...
              options: [
                {
                  name: 'ckp',
                  type: 'string',
                  value: 'vision51_v51VAE_safetensors'
                }
              ],
              input: [],
              output: [
...
"167" : "jewelery, diamonds, crystals, marbles",
                ...
                },
                {
                  name: 'pre_text',
                  type: 'string',
                  value: 'photoreealistic photography, to'
                },
                {
                  name: 'app_text',
                  type: 'string',
                  value: 'black white interior,top angl'
                },
              ...
              options: [
                {
                  name: 'control_net_name',
                  type: 'string',
                  value: 'control_v11p_sd15_depth.pth'
                }
              ],
              input: [],
              output: [{ name: 'CONTROL_NET', type: 1 }]
              ...
              options: [
                {
                  name: 'vae_name',
                  type: 'string',
                  value: 'vae-ft-mse-840000.safetensors'
                }
              ],
              input: [],
              ...
              options: [
                {
                  name: '_image',
                  type: 'string',
                  value: '/demo-images/open-all-close-all.gif'
                }
              ],
      ...
      const getModelOutputParam = (model, outputParamName) => {
        return model.data.output.find(
          (param) => param.name === outputParamName
        )
      }
      ...
      __graph_json_data.elementLines = modelRelations.map((relation) => {
        ...
        const outputParam = getModelOutputParam(
          outputModel,
          relation.output_param_name
        )
        const inputModel = getModel(relation.input_model)
        const inputParam = getModelInputParam(
          inputModel,
          relation.input_param_name
        )
    ...
    onNodeClick(node) {
      ...
      elLines.forEach((line) => {
        ...
        const refed =
          line.from.includes(keywords) || line.to.includes(keywords)
